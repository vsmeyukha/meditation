...
export async function setEnabledAmbient(
  enabled: boolean,
  volumes: Record<ChannelName, number>,
): Promise<void> {
  const e = ensureEngine();
  if (enabled) {
    await e.ctx.resume();
...
export function setAmbientVolumes(volumes: Record<ChannelName, number>): void {
  ...
  if (e.nodes.bowlsGain) {
    e.nodes.bowlsGain.gain.linearRampToValueAtTime(
      bowlsTarget,
      e.ctx.currentTime + 0.2,
    );
  }
}
