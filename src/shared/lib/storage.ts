...
export function logPractice(): void {
  ...
  const prev = readLocalStorage<StreakState>(storageKeys.streak, {
    streakDays: 0,
  });
  let streakDays = prev.streakDays ?? 0;
  const prevDate = prev.lastPracticeISO
    ? new Date(prev.lastPracticeISO)
    : undefined;
  if (!prevDate) {
    streakDays = 1;
  } else {
    ...
    const diffDays = Math.round(
      (today.getTime() - prevDay.getTime()) / (1000 * 60 * 60 * 24),
    );
    if (diffDays === 0) {
  ...
  writeLocalStorage(storageKeys.streak, {
    streakDays,
    lastPracticeISO: now.toISOString(),
  });
}
