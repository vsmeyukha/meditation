import { readLocalStorage, storageKeys, writeLocalStorage, MixerState } from "./storage"
...
class AmbientStore {
  ...
  setVolume(key: keyof Omit<MixerState, "enabled">, value: number) {
    ...
    if (this.state.enabled) {
      setAmbientVolumes({ rain: this.state.rain, stream: this.state.stream, bowls: this.state.bowls })
    }
    this.notify()
  }
}
