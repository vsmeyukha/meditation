...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { mood, energy, topic } = body as { mood?: string; energy?: string; topic?: string }
    ...
    const resp = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: "gpt-4o-mini",
        messages: [
          { role: "system", content: "You write meditations that are short, kind, and non-clinical." },
    ...
    interface ChatChoice { index: number; message: ChatMessage }
